...
スクリーンショットはデモデータによる画面例です。プロジェクト名や件数は実際の利用指標ではありません。
...
登録済みのローカルリポジトリについて、Git bundle と未コミットファイルを保存します。定期・大規模変更バックアップは未変更の内容を省略し、手動作成にも対応。リポジトリごとに最大三世代を保持します。

アプリ内 Agent の書き込み前に復元ポイントを作成できます。外部 Agent、ターミナルやスクリプトによる削除、変更の消失、参照の巻き戻り、利用不能を監視し、理由とパスを表示します。バックアップ内のファイル確認・比較・選択エクスポート、新しいディレクトリへの復元、保存先変更時の移行に対応します。

停電時の復旧は直近の完了済みポイントが基準です。内容と一覧をディスクに同期してから完了マーカーを同期し、その後で古い世代を整理します。中断した書き込みは次回起動時に処理します。バックアップ後の変更、エディタ内の未保存内容、除外ファイルの復旧は保証できません。外部アプリの全コマンドを遮断する機能ではありません。

<a id="monitoring"></a>
### メニューバーから全リポジトリを確認

メインウインドウとは別に、全リポジトリまたは一つを選べます。変更、上流状態、復元ポイント、Actions、目標、日別活動を表示。閉じた状態でも範囲・変更数・警告が見え、展開パネルはスクロールでき、アプリのテーマに従います。活動はコミットと観測した変更で、作業時間ではありません。

設定で終了後の監視継続を有効にすると、独立したヘルパーが監視、定期バックアップ、大規模変更バックアップ、リポジトリ保護を設定どおり引き継ぎます。再度アプリを開くと処理を返し、二重スキャンを避けます。既定では無効で、macOS の承認が必要な場合があります。

全体・各チャンネル・メニューバー表示・間隔は設定で変更できます。アイコンを隠しても有効なバックグラウンドバックアップは停止しません。全体または保護を無効にすると対応する処理が停止します。

<a id="goals"></a>
### 中断後も続けられる目標

「コミットしてプッシュ」「PR 作成」「リリース公開」またはカスタム目標を選びます。変更、Issue、PR、失敗したチェックからも作成でき、進捗を優先表示し、詳細と履歴を展開できます。
...
Codex CLI、Claude Code、Gemini CLI、OpenCode、DeepSeek Harness（dsh）、Cursor Agent、GitHub Copilot CLI、Qwen Code、カスタム CLI に対応。内蔵の Git 指針はステージのレビュー、コミット案、競合、分岐の整理、履歴復旧、健全性、リリース確認を扱い、LFS、hooks、署名、同期のエラー出力も調査に渡せます。

プロジェクト、翻訳、検索、インストールは別の実行経路です。README は書き換え結果をプレビューして適用。Issue・PR 返信は議論と差分から編集可能な下書きを作り、確認後に送信します。利用中の CLI とモデル設定を引き継げます。

OpenAI 互換 API または DeepSeek API を直接設定でき、API モードでは CLI のインストールは不要です。プロジェクトと翻訳で接続先・モデルを分け、モデル一覧、能力確認、ストリーミング応答を利用できます。API キーは macOS キーチェーンに保存。API Agent はプロジェクトの読み取り、コマンド実行、制御されたパスへの書き込みに対応し、翻訳には書き込みツールを渡しません。
...
| Agent・翻訳・Agent インストール | 設定済み CLI、または OpenAI 互換 / DeepSeek API。選択タスクに対応したモデルと権限 |
...
ローカル保存は完全オフラインを意味しません。GitHub へ通信し、Agent・翻訳に必要な文脈を設定した CLI または API へ渡します。その後の扱いはツールとモデルサービスに依存します。送信内容を確認し、コマンド・下書き・カプセルに認証情報を入れないでください。システムディレクトリの変更には macOS の認可が必要です。
...
ロードマップはソースとバージョン記録に基づき、破線は計画です。Star 図は 2026-09-12 UTC 時点の Stargazers の日時を累計し、取り消された Star は含みません。クリックでオンライン記録を開けます。
