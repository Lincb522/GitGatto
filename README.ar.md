...
### تنظيم التغييرات والأدلة

- يجمع مركز التغييرات التعديلات حسب الملف أو مقطع Diff ويمكنه إنشاء عدة commits ذرية. يتحقق أولًا من بصمة المستودع وينشئ نقطة استعادة؛ وإذا فشل commit أو التحقق يعيد HEAD وحدود staging الأصلية.
- يتتبع أصل الشفرة سطر الملف إلى commit، ويضيف Pull Request وIssues وReviews وChecks المرتبطة عند توفر GitHub CLI.
- تجمع كبسولات إعادة الإنتاج التصحيح والملفات غير المتعقبة وcommit الأساس والأمر الفاشل والمخرجات وإصدارات الأدوات في ملف `.gatto`. يتحقق الاستيراد من المحتوى ثم يستعيده داخل worktree معزول.
- يسجل دفتر النشاط تغييرات مراجع Git وحالة الملفات مع عمليات Agent التي كان مجلد عملها داخل المستودع. تُعرض درجة الارتباط بوضوح ولا يُعامل الارتباط على أنه مسؤولية مثبتة.
